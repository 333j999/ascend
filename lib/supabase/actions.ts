...
/**
 * Supabase JS doesn't throw on database errors — it returns `{ error }`.
 * If we don't check it, server actions silently swallow failures and the
 * UI optimistically pretends everything worked. This wrapper makes any
 * error visible to the caller (which surfaces in the client component).
 */
function ensureOk<T extends { error: any }>(res: T, op: string): T {
  if (res.error) {
    // Re-throw with a readable message so the client useTransition catches it
    throw new Error(`${op}: ${res.error.message ?? "unknown error"}`);
  }
  return res;
}
...
export async function addTransaction(input: {
  ...
  const supabase = createClient();
  ensureOk(
    await supabase.from("transactions").insert({ user_id: userId, ...input }),
    "addTransaction",
  );
  revalidatePath("/app", "layout");
}
...
export async function deleteTransaction(id: string) {
  await requireUserId();
  const supabase = createClient();
  ensureOk(await supabase.from("transactions").delete().eq("id", id), "deleteTransaction");
  revalidatePath("/app", "layout");
}
...
export async function updateTransaction(id: string, input: {
  ...
  await requireUserId();
  const supabase = createClient();
  ensureOk(await supabase.from("transactions").update(input).eq("id", id), "updateTransaction");
  revalidatePath("/app", "layout");
}
...
export async function addSavingsGoal(input: {
  ...
  const supabase = createClient();
  ensureOk(
    await supabase.from("savings_goals").insert({ user_id: userId, ...input }),
    "addSavingsGoal",
  );
  revalidatePath("/app", "layout");
}
...
export async function contributeToGoal(goalId: string, amount: number) {
  ...
  if (error) throw new Error(`contributeToGoal.fetchGoal: ${error.message}`);
...
export async function deleteSavingsGoal(id: string) {
  await requireUserId();
  const supabase = createClient();
  ensureOk(await supabase.from("savings_goals").delete().eq("id", id), "deleteSavingsGoal");
  revalidatePath("/app", "layout");
}
...
export async function deleteMission(id: string) {
  await requireUserId();
  const supabase = createClient();
  ensureOk(await supabase.from("missions").delete().eq("id", id), "deleteMission");
  revalidatePath("/app", "layout");
}
...
export async function deleteHabit(id: string) {
  await requireUserId();
  const supabase = createClient();
  ensureOk(await supabase.from("habits").delete().eq("id", id), "deleteHabit");
  revalidatePath("/app", "layout");
}
...
export async function addJournalEntry(input: {
  ...
  const supabase = createClient();
  ensureOk(
    await supabase.from("journal_entries").insert({ user_id: userId, ...input }),
    "addJournalEntry",
  );
  revalidatePath("/app", "layout");
}
